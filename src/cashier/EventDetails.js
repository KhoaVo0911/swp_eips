...
import { CardAction, PostSearchCardAsyncApi, PutCardAsyncApi, PutDepositAsyncApi, PutWithdrawAsyncApi, getCardAsyncApi } from '../services/card/cardSlice';
...
import { EventAction, getEventCashierAsyncApi } from '../services/event/eventSlice';
...
export default function EventDetails() {
    ...
    const handleUsernameChange = (index, newValue) => {
        setcardList((prevCardList) => {
            ...
            updatedCardList[index] = { ...updatedCardList[index], username: newValue };
    ...
    const handlePhoneNumberChange = (index, value) => {
        setcardList((prevCardList) => {
            const updatedCardList = [...prevCardList];
            updatedCardList[index] = { ...updatedCardList[index], phoneNumber: value };
            return updatedCardList;
        });
    };
                        ...
                        <div className="custom-block bg-white">
                            <div className="tab-content" id="myTabContent">
                                ...
                                >
                                    ...
                                    <tbody>
                                        {CardList && cardLisst.map((item, index) => {
                                            return (
                                                <tr key={index}>
                                                    <th className='pt-4 text-xl w-10'>{item.id}</th>
                                                    <th className='pt-4 text-xl w-44'>
                                                        {isUpdate !== index ? item.username
                                                            : <div className='w-44 items-center text-center justify-center mx-auto'>
                                                                <input className="form-control w-32" value={item.username}
                                                                    onChange={(e) => handleUsernameChange(index, e.target.value)} />
                                                            </div>}
                                                    </th>
                                                    <th className='pt-4 text-xl w-44'>
                                                        {isUpdate !== index ? item.phoneNumber
                                                            : <div className='w-44 items-center text-center justify-center mx-auto'>
                                                                <input className="form-control w-32" value={item.phoneNumber}
                                                                    onChange={(e) => handlePhoneNumberChange(index, e.target.value)} />
                                                            </div>
                                                        }
                                                    </th>
                                                    <th className='pt-4 text-xl w-44'>
                                                        {isUpdate !== index ? item.balance
                                                            : <div className='w-44 items-center text-center justify-center mx-auto'>
                                                                <input className="form-control w-32" value={item.balance}
                                                                    onChange={(e) => handleBalanceChange(index, e.target.value)} />
                                                            </div>}
                                                    </th>
                                                    <th className='pt-4 text-xl w-44'>
                                                        {item.status == true ? <PublicIcon /> : item.status == false ? <PublicOffIcon /> : null}
                                                    </th>
                                                    ...
                                                    <th component="div" className='pt-4 text-xl w-32'>
                                                        ...
                                                        <div className='flex justify-center '>
                                                            ...
                                                            <div>
                                                                {isUpdate !== index ?
                                                                    <Tooltip onClick={() => handleOpenUpdateCardChange(item, index)} title="Edit" className='h-10 w-10'>
                                                                        <IconButton  >
                                                                            <EditIcon className="" />
                                                                        </IconButton>
                                                                    </Tooltip> :
                                                                    <div className='flex gap-3'>
                                                                        ...
                                                                        <Tooltip onClick={() => handleUpdateCardChange(item)} title="Done" className='h-10 w-10'>
                                                                            <IconButton  >
                                                                                <DoneIcon className="" />
                                                                            </IconButton>
                                                                        </Tooltip>
                                                                ...
                                                                {deposit == 1 ?
                                                                    <Tooltip onClick={() => handleClickOpenUpdate(item)} title="Done" className='h-10 w-10'>
                                                                        <IconButton  >
                                                                            <DoneIcon className="" />
                                                                        </IconButton>
                                                                    </Tooltip> : <Tooltip onClick={() => handleDepositOpen(item)} handle title="Deposit" className='h-10 w-10'>
                                                                        <IconButton >
                                                                ...
                                                                {deposit == 2 ?
                                                                    ...
                                                                    </Tooltip> : <Tooltip onClick={() => handleWithDrawOpen(item)} title="Withdraw" className='h-10 w-10'>
                                                                        <IconButton >
                                                                            <SouthIcon className="" />
                                                                        </IconButton>
                                                                    </Tooltip>}
