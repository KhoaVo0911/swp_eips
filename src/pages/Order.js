...
import { getOrderByEventAsyncApi, getOrderByShopAsyncApi } from '../services/product/productSlice';
import { GetListOrderAsyncApi, getShopByUsernameAsyncApi } from '../services/shop/shopSlice';
...
export default function Order() {
    ...
    useEffect(() => {
        dispatch(getShopByUsernameAsyncApi(userObject.username)).then((response) => {
            if (response.payload != undefined) {
                ...
                dispatch(GetListOrderAsyncApi(response.payload.id)).then((response) => {
                    if (response.payload != undefined) {
                    }
                }).catch((error) => {
                });
    ...
    const handleClickOpenOrder = (item) => {
        setOpenOrder(true);
        dispatch(getOrderByShopAsyncApi({id: item.id, shopId: shopByUsername.id})).then((response) => {
            if (response.payload != undefined) {
            }
        }).catch((error) => {
            // Handle failure case
                                    ...
                                    {OrerList.length > 0 && OrerList.map((item, index) => {
                                        return (
                                            <tr key={index}>
                                                ...
                                                <td className=''>
                                                    <Tooltip onClick={() => handleClickOpenOrder(item)} title="view">
                                                        <IconButton>
