...
import {
  EventAction,
  PostImgEventAsyncApi,
  getEventByIdAsyncApi,
  getEventImgListAsyncApi,
...
function EventAdmin() {
  ...
  const handleClickOpenOrder = (data) => {
    setOpenOrder(true);
    dispatch(GetListOrderAsyncApi(data.id))
      .then((response) => {
  ...
  useEffect(() => {
    ...
    dispatch(getEventByIdAsyncApi(param.id))
      .then((response) => {
        if (response.payload != undefined) {
        }
      })
    ...
    dispatch(getOrderByEventAsyncApi(param.id))
      .then((response) => {
        if (response.payload != undefined) {
        }
      })
    ...
    dispatch(getShopAsyncApi(param.id))
      .then((response) => {
        if (response.payload != undefined) {
    ...
    dispatch(getEventImgListAsyncApi(param.id))
      .then((response) => {
        if (response.payload != undefined) {
        }
      })
  ...
  const hanleClickCard = () => {
    setOpenUpdate(false);
    setUpdate50Card(0);
    dispatch(PostCardAsyncApi({ eventId: param.id }))
      .then((response) => {
        if (response.payload != undefined) {
          dispatch(getCardAsyncApi(param.id));
        }
  ...
  const hanleClickUpdateStatus = () => {
    ...
    setUpdate50Card(0);
    dispatch(
      PutStatusAsyncApi({ id: updateState.id, status: !updateState.status })
    )
      .then((response) => {
        if (response.payload != undefined) {
          dispatch(getCardAsyncApi(param.id));
  ...
  const hanleClick50Card = () => {
    setOpenUpdate(false);
    setUpdate50Card(0);
    dispatch(PostCard50AsyncApi({ eventId: param.id }))
      .then((response) => {
        if (response.payload != undefined) {
          dispatch(getCardAsyncApi(param.id));
        }
      ...
      uploadTask.on(
        ...
        () => {
          ...
          getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
            ...
            if (isUpdate == false) {
              dispatch(PostShopAsyncApi(DataBody))
                .then((response) => {
                  setOpen(false);
                  formik.setValues({
                    id: "",
              ...
              dispatch(PutShopAsyncApi(DataBodyUpdate))
                .then((response) => {
                  setOpen(false);
                  formik.setValues({
                    id: "",
    ...
    uploadTask.on(
      ...
      () => {
        ...
        getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
          ...
          dispatch(PostImgEventAsyncApi(DataBody))
            .then((response) => {
              SetClick(false);
              setSelectedImage();
              setIsImg(false);
            ...
            <div className="row my-4">
              <div className="grid grid-cols-2 gap-5">
                {/* <div className="mb-10">
                 ...
                 <div
                  className="mb-10"
                  style={{ height: "80%", width: "60%", marginLeft: "200px" }}
                >
                  <Slider>
                    {eventListImg &&
                      eventListImg.map((item, index) => {
                        return <img src={item.img} className="h-64 w-44 p-1" />;
                      })}
                  </Slider>{" "}
                  :
                ...
                <div className="pb-5">
                  {eventById && (
                    <div className="bg-white px-4 py-1 mx-36">
                      <Typography
                        gutterBottom
                        variant="h4"
                      ...
                      <Typography variant="h6">
                        ...
                        {eventById.beginDate &&
                          parseTimestamp(eventById.beginDate)}
                      </Typography>
                      <Typography variant="h6">
                        <strong>End Date:</strong>{" "}
                        {eventById.endDate && parseTimestamp(eventById.endDate)}
                      </Typography>
                ...
                <CustomTabPanel value={value} index={1}>
                  <div className="flex gap-2">
                    <button
                      className="nav-link form-control mb-3"
                      style={{ textAlign: "center" }}
                      onClick={() => handleClickOpenUpdate(1)}
                    >
                      Create Card
                    </button>
                    <button
                      className="nav-link form-control mb-3"
                      style={{ textAlign: "center" }}
                      onClick={() => handleClickOpenUpdate(2)}
                    >
                      Create 50 Cards
                    </button>
                  </div>
                          ...
                          CardList.map((item, index) => {
                            return (
                              <tr key={index}>
                                ...
                                <td>{item.id}</td>
                                <td>
                                  {item.username == null ? null : item.username}
                                </td>
                                <td>
          ...
          <DialogContent dividers>
            ...
            <div className="max-w-5xl my-2 mx-auto">
              <TextField
                ...
                id="outlined-basic"
                error={formik.touched.id && formik.errors.id ? true : undefined}
