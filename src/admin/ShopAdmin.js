...
import {
  GetAccountNotRelationAsyncApi,
  PostAccountAsyncApi,
  PostAccountForSaleAsyncApi,
  PostShopAccountSetAsyncApi,
  accountAction,
...
function ShopAdmin() {
  ...
  useEffect(() => {
    ...
    dispatch(getEventByIdAsyncApi(additionalData.id))
      .then((response) => {
        if (response.payload != undefined) {
        }
      })
      .catch((error) => {});
    dispatch(getProductAsyncApi(param.id))
      .then((response) => {
        if (response.payload != undefined) {
  ...
  const handleSetAccountShop = () => {
    dispatch(
      PostShopAccountSetAsyncApi({ username: accountSet, shopId: param.id })
    );
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      dispatch(PostAccountForSaleAsyncApi(DataBody))
        .then((response) => {
          setOpen(false);
          formik.setValues({
            username: "",
          ...
          <main className="main-wrapper ms-sm-auto py-4 px-md-4 border-start">
            <div className="row my-4">
              <div className="col-lg-6 col-6 bg-white mb-4 p-2">
                ...
                >
                  <h3 className="h4 mb-0">
                    {eventById.name} - {additionalData.shopDetail.name}
                  </h3>
                  <div className="grid grid-cols-2">
                    <div>
                      <img
                        src={additionalData.shopDetail.image}
                        className="h-32 w-32 mx-auto  mt-[18px]"
                      />
                    </div>
                    <div className="text-left">
                      ...
                      <Typography variant="h6">
                        ...
                        {additionalData.shopDetail.eventId}
                      </Typography>
                      <Typography variant="h6">
              ...
              <div className="col-lg-6 col-6" style={{ paddingRight: "300px" }}>
                ...
                <div>
                  ...
                  {openSet == true ? (
                    <>
                      {" "}
                      <FormControl className="w-full bg-white mb-3">
                        ...
                        >
                          {AccountNotRelation &&
                            AccountNotRelation.map((item, index) => {
                              return (
              ...
              <div className="custom-block bg-white">
                <div className="flex gap-5">
                  ...
                  <div>
                    <p className="text-2xl font-bold mt-[10px]">
                      Total Revenue: {parseToVND(Revenue) + " VNĐ"}
                    </p>
                  </div>
                </div>
            ...
            <div className="col-2">
              <Link
                className="nav-link form-control mb-3"
                style={{ textAlign: "center" }}
                to={`/EventAdmin/${additionalData.id}`}
              >
                Previous
              </Link>
            </div>
